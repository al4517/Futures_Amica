...
def main():
    ...
    with Board() as board:
        ...
        aiy.voice.tts.say('We\'ve almost finished War and Peace on Audible...only 1 hour left. I could read you that if you want?')
        text = client.recognize(language_code=args.language,
                                hint_phrases=hints)
        
        aiy.voice.tts.say('Well you tend to finnish shorter books much faster. How about a George Orwell next?')
        text = client.recognize(language_code=args.language,
                                hint_phrases=hints)
